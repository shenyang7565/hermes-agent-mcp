...
class MCPServerTask:
    ...
    __slots__ = (
        ...
        "_stdio_exited_unexpectedly", "_last_health_check_ts", "_health_check_task",
    )
    ...
    def __init__(self, name: str):
        ...
        # Supervision state
        self._stdio_exited_unexpectedly = False  # True when subprocess died without shutdown
        self._last_health_check_ts: float = 0.0  # Unix timestamp of last successful check
        self._health_check_task: Optional[asyncio.Task] = None  # Periodic ping task

    def _is_http(self) -> bool:
        """Check if this server uses HTTP transport."""
        return "url" in self._config

    def is_healthy(self, grace_seconds: float = 30.0) -> bool:
        """Return True when the server is connected AND has responded recently.

        Args:
            grace_seconds: how many seconds to tolerate without a health check
                           before marking the server unhealthy.  Default 30 s.
        """
        if self.session is None:
            return False
        if self._stdio_exited_unexpectedly:
            return False
        now = time.time()
        if now - self._last_health_check_ts > grace_seconds:
            # No successful check recorded yet — tolerate startup window
            if self._last_health_check_ts == 0.0:
                return True
            return False
        return True

    async def _health_check_loop(self, grace_seconds: float = 30.0):
        """Background coroutine: ping the server periodically to detect hangs."""
        import httpx

        await asyncio.sleep(5.0)  # give the server time to settle after startup
        while not self._shutdown_event.is_set():
            try:
                if self._is_http():
                    url = self._config["url"].rstrip("/") + "/+health"
                    ...
                    # stdio: call a lightweight tool (ping or list_tools)
                    if self.session and not self.session._is_closed:
    ...
    async def _run_stdio(self, config: dict):
        ...
        self._stdio_exited_unexpectedly = False  # reset on each new connection attempt
        async with stdio_client(server_params) as (read_stream, write_stream):
            ...
            async with ClientSession(read_stream, write_stream, **sampling_kwargs) as session:
                ...
                self._health_check_task = asyncio.ensure_future(
                    self._health_check_loop(grace_seconds=30.0)
                )
                self._ready.set()
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # Mark unexpected exit so is_healthy() returns False during backoff
                self._stdio_exited_unexpectedly = True
...
def get_mcp_status() -> List[dict]:
    ...
    for name, cfg in configured.items():
        ...
        if server and server.is_healthy(grace_seconds=45.0):
            entry = {
                "name": name,
                "transport": transport,
                ...
                "connected": True,
                "healthy": True,
                "last_check_ts": server._last_health_check_ts,
            }
            if server._sampling:
            ...
            entry = {
                ...
                "connected": False,
                "healthy": False,
                "last_check_ts": server._last_health_check_ts if server else 0.0,
            }
            result.append(entry)
...
def _get_active_servers() -> List[str]:
    """Return names of all MCP servers that are currently connected and healthy."""
    with _lock:
        return [
            name for name, srv in _servers.items()
            if srv.is_healthy(grace_seconds=45.0)
        ]


def _get_server_info(name: str) -> dict:
    """Return a dict with status/tool_count for one server."""
    with _lock:
        srv = _servers.get(name)
    if not srv:
        return {"status": "not configured", "tool_count": 0}
    if srv.is_healthy(grace_seconds=45.0):
        return {
            "status": "healthy",
            "tool_count": len(srv._registered_tool_names) if hasattr(srv, "_registered_tool_names") else len(srv._tools),
        }
    return {
        "status": "unhealthy" if srv.session else "disconnected",
        "tool_count": 0,
    }
