...
from event_stream import set_current_session
...
class GatewayRunner:
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        # ── Layer 1: Deterministic built-in commands (no LLM, zero token cost) ──
        # Mimics Shell's strtok parsing — deterministic dispatch before LLM.
        raw_text = event.text or ""
        # DEBUG
        import os
        if os.environ.get("HERMES_DEBUG_CMD"):
            print(f"[DEBUG Layer1] raw_text={raw_text!r}", flush=True)
        if raw_text.startswith("/"):
            # Split with strtok logic
            parts = raw_text.split()
            cmd = parts[0][1:]  # strip the leading '/'
            args = parts[1:] if len(parts) > 1 else []

            if cmd == "status" or cmd == "st":
                return await self._handle_status_command(event, extended=True)

            if cmd == "memory":
                return await self._handle_memory_command(event)

            if cmd == "tools":
                return self._handle_tools_command(event)

            if cmd == "mcp":
                # Pass "mcp list" style args to existing reload handler
                if args and args[0] == "list":
                    return self._handle_mcp_list_command(event)

        # ── Layer 2: Normal command dispatch (/new, /help, etc.) ──
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        set_current_session(session_entry.session_id)
    ...
    async def _handle_status_command(self, event: MessageEvent, extended: bool = False) -> str:
        """Handle /status command (extended shows MCP/tools/memory stats)."""
        source = event.source
        session_entry = self.session_store.get_or_create_session(source)
        ...
        lines = [
            "📊 **Hermes Status**",
            "",
            f"**Session:** `{session_entry.session_id[:16]}...`",
            f"**Platforms:** {', '.join(connected_platforms) or 'none'}",
            f"**Agent:** {'⚡ Running' if is_running else '⏸ Idle'}",
            f"**Tokens:** {session_entry.total_tokens:,}",
        ]

        # Extended status: MCP servers, tools, memory
        if extended:
            # MCP servers
            try:
                from tools.mcp_tool import _get_active_servers
            ...
            # Tool call stats from benchmark
            lines.append("")
            lines.append("**🛠 Tools (this session):**")
            try:
                import json
                bench_path = os.path.expanduser("~/.hermes/benchmark/tool_calls.jsonl")
                if os.path.exists(bench_path):
                    ...
                    lines.append("  (benchmark not active)")
            except Exception:
                lines.append("  (stats unavailable)")

            # Mem0 memory
            lines.append("")
            lines.append("**🧠 Memory (Mem0):**")
            try:
                mem_dir = os.path.expanduser("~/.hermes/mem0")
                if os.path.exists(mem_dir):
                    import glob
                    vec_file = os.path.join(mem_dir, "vectors.json.json")
                    db_file = os.path.join(mem_dir, "memory.db")
                    vec_count = 0
                    ...
                    if os.path.exists(db_file):
                        ...
                        conn.close()
                    lines.append(f"  Vector: {vec_count} | KV: {kv_count}")
                else:
    ...
    def _handle_tools_command(self, event: MessageEvent) -> str:
        """Handle /tools — list all registered tools (no LLM)."""
        ...
        for ts, names in sorted(by_toolset.items()):
            lines.append(f"**{ts}** ({len(names)})")
            for n in sorted(names)[:15]:
                lines.append(f"  • {n}")
            if len(names) > 15:
                lines.append(f"  ... and {len(names)-15} more")
            lines.append("")
        lines.append(f"_Total: {len(registry._tools)} tools_")
        return "\n".join(lines)

    def _handle_mcp_list_command(self, event: MessageEvent) -> str:
        """Handle /mcp list — show MCP server status (no LLM)."""
        try:
            from tools.mcp_tool import _get_active_servers, _get_server_info
            servers = _get_active_servers()
            lines = ["**🔌 MCP Servers**", ""]
            if servers:
                for name in sorted(servers):
                    info = _get_server_info(name)
                    status = info.get("status", "unknown") if info else "?"
                    tools = info.get("tool_count", "?") if info else "?"
                    lines.append(f"  • `{name}` — {status}, {tools} tools")
            else:
                lines.append("  (none connected)")
            return "\n".join(lines)
        except Exception as e:
    ...
    async def _handle_memory_command(self, event: MessageEvent) -> str:
        """Handle /memory — show Mem0 three-layer memory stats."""
        lines = ["**🧠 Mem0 Memory**", ""]
        mem_dir = os.path.expanduser("~/.hermes/mem0")
        if not os.path.exists(mem_dir):
            return "🧠 **Memory:** not initialized (send a message to activate)"
        ...
        try:
            ...
            if os.path.exists(db_file):
                ...
                cur = conn.execute("SELECT COUNT(*) FROM memories")
                kv_count = cur.fetchone()[0]
                cur2 = conn.execute("SELECT key FROM memories LIMIT 20")
                kv_keys = [r[0] for r in cur2.fetchall()]
                conn.close()

            epi_file = os.path.join(mem_dir, "episodes.json")
            epi_count = 0
            node_count = 0
            if os.path.exists(epi_file):
                with open(epi_file) as f:
                    data = _json.load(f)
                    epi_count = len(data)
                    node_count = sum(len(ep.get("nodes", [])) for ep in data)

            lines.append(f"**Layer 1 — Vector:** {vec_count} vectors")
            lines.append(f"**Layer 2 — KV:** {kv_count} memories")
            if kv_keys:
                lines.append(f"  Keys: {', '.join(kv_keys[:8])}")
            lines.append(f"**Layer 3 — Graph:** {epi_count} episodes, {node_count} nodes")
            lines.append("")
            lines.append("_Send a message to add tool calls to memory_")
            return "\n".join(lines)
        except Exception as e:
