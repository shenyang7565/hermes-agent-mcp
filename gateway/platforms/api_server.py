...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _handle_mcp_list(self, request: "web.Request") -> "web.Response":
        """GET /mcp/list — MCP server status (no LLM, zero token cost)."""
        try:
            ...
            servers = [
                {
                    "name": s.get("name"),
                    "status": "healthy" if s.get("healthy") else "disconnected",
            ...
            return web.json_response({
                "servers": servers,
                "active_count": sum(1 for s in servers if s["status"] == "healthy"),
            })
        except Exception as e:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app.router.add_get("/mcp/list", self._handle_mcp_list)
